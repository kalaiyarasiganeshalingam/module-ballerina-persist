...
public isolated client class Client {
    *persist:AbstractPersistClient;
    ...
    public isolated function init() returns persist:Error? {
        ...
        self.persistClients = {
            [MANUFACTURE] : check new (dbClient, self.metadata.get(MANUFACTURE)),
            [PRODUCT] : check new (dbClient, self.metadata.get(PRODUCT))
        };
    }
    ...
    isolated resource function get products/[string id](ProductTargetType targetType = <>) returns targetType|persist:Error = @java:Method {
        'class: "io.ballerina.stdlib.persist.sql.datastore.MySQLProcessor",
        name: "queryOne"
    } external;

    isolated resource function post products(ProductInsert[] data) returns string[]|persist:Error {
        psql:SQLClient sqlClient;
        lock {
    ...
    isolated resource function put products/[string id](ProductUpdate value) returns Product|persist:Error {
        psql:SQLClient sqlClient;
        lock {
            sqlClient = self.persistClients.get(PRODUCT);
        }
    ...
    isolated resource function delete products/[string id]() returns Product|persist:Error {
